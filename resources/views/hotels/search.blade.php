...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <form action="{{ route('hotels.search') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 items-end">
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="city" value="{{ $params['city'] ?? '' }}" placeholder="City name..." class="w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-10 pr-4 text-sm text-white placeholder-gray-500 focus:outline-none input-glow focus:border-primary-500/50 transition-all">
                </div>
            </div>
            <div>
            ...
            <div>
                <label class="block text-xs font-medium text-gray-400 mb-1.5 ml-1">Guests</label>
                <select name="guests" class="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-sm text-white focus:outline-none input-glow focus:border-primary-500/50 transition-all appearance-none">
                    @for($i = 1; $i <= 6; $i++)
                        <option value="{{ $i }}" {{ ($params['guests'] ?? 1) == $i ? 'selected' : '' }} class="bg-dark-900">{{ $i }} Guest{{ $i > 1 ? 's' : '' }}</option>
                        @endfor
                </select>
            </div>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-400 mt-1">{{ count($hotels) }} {{ Str::plural('result', count($hotels)) }} found</p>
        </div>
    </div>
    ...
    <div class="glass rounded-2xl p-12 text-center">
        ...
        <h3 class="text-lg font-semibold text-white mb-2">No hotels found</h3>
        <p class="text-gray-400 text-sm">Try adjusting your search criteria or search for a different destination.</p>
        ...
        <a href="{{ route('hotels.show', $hotel['id']) }}" class="glass rounded-2xl overflow-hidden card-hover shine-effect group">
            <div class="relative h-48 overflow-hidden">
                ...
                <div class="absolute top-3 left-3 px-2.5 py-1 bg-red-500/90 rounded-lg text-xs font-bold text-white">
                    -{{ round((1 - $hotel['price_per_night'] / $hotel['original_price']) * 100) }}% OFF
                </div>
                @endif
            ...
            <div class="p-5">
                ...
                <div class="flex flex-wrap gap-1.5 mb-4">
                    ...
                    <span class="px-2 py-0.5 rounded-md text-[11px] font-medium bg-white/5 text-gray-400">+{{ count($hotel['amenities']) - 4 }} more</span>
                    @endif
                </div>
